package com.and.netease.layout;
...
public class SlideImageLayout {
	...
	public View getLinearLayout(View view, int width, int height) {
		...
		LinearLayout.LayoutParams linerLayoutParames = new LinearLayout.LayoutParams(width, height,
				1);
		// 这里最好也自定义设置，有兴趣的自己设置。
	...
	public ImageView getCircleImageLayout(int index) {
		...
		if (index == 0) {
			// 默认选中第一张图片
			mImageViews[index].setBackgroundResource(cn.buaa.myweixin.R.drawable.dot_selected);
		} else {
	...
	private class ImageOnClickListener implements OnClickListener {
		...
		public void onClick(View v) {
			Toast.makeText(mContext, mParser.getSlideTitles()[pageIndex], Toast.LENGTH_SHORT)
					.show();
